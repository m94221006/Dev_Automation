...
class Telnet_Console(object):
            ...
            if mode == "router" :
               ...
               if len(readstring) != 0 :
                  ...
                  if len(readstring) != 0 :
                      ...
                      for i in range(0, len(userInfo), 1) :
                           ...
                           if portTmp1 in userInfo[i] :
                              ...
                              time.sleep(1)
                              readstring =  self.telnet.read_until('router', timeout=3)
                              print "kill user success ==> " + self.ipaddress + ":" + self.port
                              killTmp= True
                      if killTmp != True :
                          print "No user in"
                  self.telnet.close()
                ...
                if "Port not connected" in str(readstring) :
                    ...
                    print "disconnect " + self.port + " success"

                self.telnet.close()
                return True
            else :
                print "please check mode (cisco or server)"
                return False

        except :
            print "connect to router or server fail"
            self.telnet.close()
            return False
    ...
    def send_command(self,command,timeout,checkResponse):
        try:
            ...
            for command in commandlist:
                if 'save' not in command and 'update' not in command:
                    if command not in readResult:
                        print "[command]%s not success, set again."%(command)
                        self.telnet.write((chr(int(3))).encode('ascii'))
                        self.telnet.write((command+"\n").encode('ascii'))
